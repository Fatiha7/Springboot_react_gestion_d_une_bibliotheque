...
const EditeRevue = ({id,forceUpdate}) => {
  const [isOpen, setIsOpen] = useState(false);
    ...
    try{
        const response = await RevuesService.getRevuesById(id);
        console.log(id)
    ...
    const updateRevues = (e) =>{
        e.preventDefault();
        console.log(Revues)
        RevuesService.updateRevues(Revues,id).then(
    (response) => {
  ...
  return (
   ...
   <div className='min-h-screen px-4 text-center '  >
       ...
       <div className='inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-left align-middle border-4 transition-all transform bg-gray-100 shadow-xl rounded-md'>
           ...
           <Dialog.Title as='h3' className=" text-4xl font-semibold tracking-wider text-center font-serif leading-6 text-gray-800">Update Journal</Dialog.Title>
       ...
       <div className='items-center justify-center h-14 w-full ml-2 mt-6 my-4 space-x-4'>
         <button onClick={updateRevues} className='rounded text-white font-semibold font-serif justify-center bg-gray-600 hover:bg-gray-800 hover:text-white mt-4 py-4  px-16'>Save</button> 
